            ...
            for gateway in gateways:
                ...
                if connections is not None:
                    ...
                    for connection in connections:
                        if ("SDK-PY" in connection.get("name")) and (
                            "delet" not in connection.get("status")):
                            if connection.get("network_type") == "gre_tunnel":
                                self.delete_resource_test(gateway_id=gateway_id, 
                                    conn_id=connection.get("id"))
                            else:
                                conn_ids.append(connection.get("id"))

                    # Deletes Connections from other types.
                    for id in conn_ids:
                        self.delete_resource_test(
                            gateway_id=gateway_id, conn_id=id)
              
                # Deletes current gateway
                if ("SDK-PY" in gateway.get("name")) and (
                    "delet" not in gateway.get("status")):
                    self.delete_resource_test(
                        gateway_id=gateway_id, conn_id="")
...
###############################################################################
    ...
    def test_03_transit_gateway_connection_actions(self):
        ...
        for conn in conns:
            if conn["network_type"] == "vpc" and conn["id"] == vpc_conn_id:
                vpc_found = True
            elif conn["network_type"] == "classic" and conn["id"] == classic_conn_id:
                classic_found = True
            elif conn["network_type"] == "directlink" and conn["id"] == dl_conn_id:
                dl_found = True
            elif conn["network_type"] == "gre_tunnel" and conn["id"] == gre_conn_id:
                gre_found = True
...
###############################################################################
    ...
    def delete_resource_test(self, gateway_id, conn_id):
        count = 0 
        response = None
    ...
    def is_resource_available(self, gateway_id, conn_id):
        count = delay = 0
        input_id = gateway_id

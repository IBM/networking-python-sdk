...
class TestDirectLinkProviderV2(unittest.TestCase):
    ...
    @unittest.skip("skipping")  # skipping this test Due to its failure
    def setUp(self):
        """ test case setup """
                ...
                if ("sdk-py-dl-provider" in record.get("name").lower() and "provisioned" in record.get('operational_status') and 'change_request' in record):
                    ...
                    if (changeRequest == "delete_gateway"):
                        self.dl.create_gateway_action(
                            id=gateway_id, action="delete_gateway_approve")
    ...
    def delete_gateway(self, gateway_id):
        response = self.dl.create_gateway_action(
            id=gateway_id, action="delete_gateway_approve")
        assert response is not None
    ...
    def test_provider_gateway_actions(self):
        timestamp = time.time()
        name = os.getenv("DL_PROVIDER_SERVICES_GW_NAME") + \
            "-" + str(int(timestamp))
        bgpAsn = 64999
        ...
        response = self.dl_provider.get_provider_gateway(
            id=provider_gateway_id)
        assert response is not None
        assert response.get_status_code() == 200
        ...
        with self.assertRaises(ApiException) as ex:
            response = self.dl_provider.update_provider_gateway(
                id=provider_gateway_id, speed_mbps=2000)
    ...
    def test_provider_gateway_actions_with_client_api(self):
        ...
        name = os.getenv("DL_PROVIDER_SERVICES_GW_NAME") + \
            "-" + str(int(timestamp))
        updatedName = os.getenv(
            "DL_PROVIDER_SERVICES_UPDATED_GW_NAME") + "-" + str(int(timestamp))
        bgpAsn = 64999
        customerAccount = os.getenv("DL_PROVIDER_SERVICES_CUSTOMER_ACCT_ID")
        speedMbps = 1000
        ...
        response = self.dl_provider.update_provider_gateway(
            id=gateway_id, speed_mbps=updatedSpeedMbps)
        assert response is not None
        assert response.get_status_code() == 200
        assert response.get_result().get("id") == gateway_id
        assert response.get_result().get("name") == updatedName
        ...
        response = self.dl_provider.update_provider_gateway(
            id=gateway_id, vlan=updatedVlan)
        assert response is not None
        assert response.get_status_code() == 200
        assert response.get_result().get("id") == gateway_id
        assert response.get_result().get("name") == updatedName
        # does not change until approved
        assert response.get_result().get("vlan") == vlan
        assert "change_request" in response.get_result()
        ...
        response = self.dl.create_gateway_action(
            id=gateway_id, action="delete_gateway_reject")
        assert response is not None
        assert response.get_status_code() == 200
    ...
    def test_provider_gateway_actions_with_client_api_bgp_ip_update(self):
        ...
        bgpIpObject = {"bgp_cer_cidr": updatedBgpCerCidr,
                       "bgp_ibm_cidr": updatedBgpIbmCidr}
